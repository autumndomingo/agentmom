...
- Fifth deploy review found four hardening gaps before the next deploy: marker-only cutovers did not explicitly stop old API/worker processes, worker restarts could kill active jobs, worker completion was a single non-idempotent POST, and the legacy mkdir state lock could survive crashes. The branch now fixes those with service-aware cutover cleanup, longer job-aware worker shutdown, retried/idempotent completion, a kernel flock, and graceful microVM shutdown before forced kill. Local Rust/Nix validation passed and the three prod NixOS toplevels built with a local Agent Mom override; this patch has not been deployed yet.
- Sixth deploy review found additional predeploy blockers: cutover stop commands could replace queued API/worker starts, public restore jobs trusted caller-supplied backup locations, multiple same-workspace jobs could run concurrently, worker HTTP service opens could race destructive lifecycle jobs, and recovery restores could be blocked by stale active source-node jobs. The branch now fails cutover stops closed with `--job-mode=fail`, canonicalizes restore jobs from catalog backups, validates backup IDs before restore temp paths, serializes same-workspace claims, holds worker-local per-workspace locks around lifecycle/service-open work, and terminalizes old workspace jobs during host-loss recovery. Validation passed: `cargo fmt --check`, `cargo test`, `nix flake check --no-build`, and all three prod NixOS toplevels built with a local Agent Mom override; this patch has not been deployed yet.
