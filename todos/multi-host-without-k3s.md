...
- Service-user cleanup on 2026-06-12: `mom-ctrl`, `mom-1`, and `mom-2` now run Agent Mom API/worker services as the `agentmom` user with agenix secrets owned by `agentmom:agentmom`. `agentmom-credential-proxy` remains root-owned where needed for CA/key material.
- Worker retry hardening on 2026-06-12: worker claim/reconcile HTTP calls now use bounded timeouts and log/retry transient API failures instead of exiting or wedging indefinitely. A fake-fleet test covers API outage/recovery, and live deployment recovered the previously stuck `mom-1` create job immediately after restart.
- Current live QA on 2026-06-12 after worker retry hardening: `mom-ctrl`, `mom-1`, and `mom-2` all switched to Agent Mom `87b8dc9`; mutating real-fleet suites passed against both `mom-1` and `mom-2`; monitor check passed with `ready_nodes=2`, `stale_nodes=0`, no queued backlog, and no recent failed jobs.
- Follow-up hardening: API service shutdown during deploy waited for systemd's default 90s stop timeout once. Add a short `TimeoutStopSec` for `agentmom-api.service` and consider making worker shutdown interrupt in-flight API calls faster than the service stop timeout.
- Idle/reconcile hardening on 2026-06-12: workers now ignore unassigned legacy workspace rows, keep `idle-stopped` workspaces cold during ordinary reconciliation, and only wake them through explicit jobs. Reconcile failure reporting is best-effort so an API/reporting hiccup does not abort the cycle.
- Current live QA on 2026-06-12 after idle/reconcile hardening: `mom-ctrl`, `mom-1`, and `mom-2` all switched to Agent Mom `f899840`; `agentmom-api.service` now reports `TimeoutStopUSec=10s`; both workers stayed alive through API restart `502` windows, then restarted cleanly during deployment; public Basic Auth routes return `401`, public worker routes return `403`, both nodes are eligible, and monitor check passes with two ready nodes and no stale/queued/failed alerts.
